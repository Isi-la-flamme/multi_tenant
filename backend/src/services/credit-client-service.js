    ...
    async createInvoice(userId, tenantId, amount, description, referenceId = null, createdBy = null) {
        ...
        if (parseFloat(credit.available_amount) < amount) {
            throw new ValidationError(
                `Crédit insuffisant. Disponible: ${currencyService.formatPrice(credit.available_amount)}`
            );
        }
        ...
        if (referenceId) {
            ...
            if (uuidRegex.test(referenceId)) {
                ...
                logger.warn(`reference_id "${referenceId}" n'est pas un UUID valide, ignoré`);
            }
        }
...
async recordPayment(userId, tenantId, invoiceId, amount, paymentMethod, reference = null, createdBy = null) {
    ...
    try {
        ...
        // Récupérer la facture avec son client_credit_id
        ...
        if (amount > amountRemaining) {
            throw new ValidationError(
                `Le montant dépasse le solde restant: ${currencyService.formatPrice(amountRemaining)}`
            );
        }
        ...
        // Enregistrer le paiement - avec CAST explicite
        const { rows } = await client.query(
...
const status = newRemaining === 0 ? 'paid' : 'partial';
    ...
    await client.query(
        ...
        SET amount_paid = $1, 
            amount_remaining = $2, 
            status = $3::text, -- Ajout du cast explicite ici
            paid_at = CASE WHEN $3::text = 'paid' THEN NOW() ELSE paid_at END, -- Et ici
            updated_at = NOW()
        WHERE id = $4::UUID`,
        [newPaid, newRemaining, status, invoiceId]
    );
        
        // Mettre à jour le crédit utilisé
        await client.query(
            `UPDATE client_credits 
             SET used_amount = used_amount - $1, 
                 available_amount = available_amount + $1,
        ...
        await client.query(
            `UPDATE wallets SET balance = $1, updated_at = NOW() WHERE id = $2::UUID`,
            [newBalance, walletId]
        );
